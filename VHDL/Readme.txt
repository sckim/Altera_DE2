...
1. document_files: contains the source files for the lab text and figures. The text files (*.tex)
   ...
   commands used to compile a source file lab_VHDL.tex and produce PDF output is:

   C:> latex lab_VHDL.tex
   C:> dvips -t letter lab_VHDL.dvi
 
   This produces a file in PostScript format called lab_VHDL.ps. Use Adobe Distiller (or some 
...
2. solutions: contains VHDL solutions to each part of each lab. For each solution all files 
   that are needed to compile the code are provided--this includes the VHDL source files, 
	Quartus II project file (qpf), and Quartus II settings (qsf) file. The sof
